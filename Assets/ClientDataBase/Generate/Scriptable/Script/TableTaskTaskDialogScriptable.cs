...
[Serializable]
public class DictionaryTableTaskTaskDialog : SerializableDictionary<int, TableTaskTaskDialog> 
{
}

public sealed class TableTaskTaskDialogScriptable : ScriptableDictionaryObject<int, TableTaskTaskDialog>
{
    private static string m_gameTableName = "TaskTaskDialog";
	
	[UnityEngine.SerializeField]
	private DictionaryTableTaskTaskDialog m_serializedTableDict = new DictionaryTableTaskTaskDialog();
    ...
    public override bool LoadGameTable(string text)
    {
        ...
        if (reader != null)
        {
            ...
            foreach (string[] splitStr in csvReader)
            {
...
#if MGAME_DEBUG
						string[] exportFieldNames = {"ID","NPC","Dialog"};
				...
				TableTaskTaskDialog table = new TableTaskTaskDialog();
				...
				var fieldIndex = -1;
					...
					}
					{
                        ...
                        trimedStr = splitStr[fieldIndex].Trim();
						table.NPC = string.IsNullOrEmpty(trimedStr) ? default(int) : (int)Convert.ChangeType(trimedStr, typeof(int));
					}
					{
                        fieldName = "Dialog";                        
                        typename = "int[]";  
	...
	public override SerializableDictionary<int, TableTaskTaskDialog> GetAllData()
    {
        return (SerializableDictionary<int, TableTaskTaskDialog>)m_serializedTableDict;
    }
}